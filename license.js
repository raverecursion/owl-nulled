...
async function EnsureLicensed(aServerId) {
  // Temporarily bypass license checking
  console.log("License check temporarily disabled.");
  return;
}
...
async function FetchTicketUnqueued() {
  try {
    let servers = await browser.incomingServer.getServersOfTypes([
      "owl",
      "owl-ews",
      "owl-eas",
    ]);
...
async function CheckLicense() {
  // Temporarily bypass ticket checking by returning a simulated valid ticket
  return {
    valid: true,
    status: "normal",
    // You can add any additional properties you need for a valid ticket simulation
  };

  // Comment out or remove the bypass code below to re-enable ticket checking
  /*
  ...
  ticket.requiresRefresh = ticket.valid && ticket.refresh < Date.now();
  if (!ticket.valid) {
...
async function VerifyTicketSignature(aTicket) {
  // Temporarily bypass signature verification and return the ticket object directly
  let ticket = JSON.parse(aTicket);
  return ticket;

  // Comment out or remove the bypass code below to re-enable signature verification
  /*
  ...
  if (!gKeyPromise) {
    ...
    gKeyPromise = crypto.subtle.importKey(
      "spki",
      keyArrayBuffer,
      algorithm,
      false,
  ...
  if (
    !(await crypto.subtle.verify(
      ...
      new TextEncoder().encode(ticket.json)
    ))
  ) {
